    ...
    <div style={{ padding: '30px', maxWidth: '1200px', margin: '0 auto' }}>
      ...
      <div style={{ 
        ...
        <div style={{ flex: 1 }}>
          <input
            type="text"
            placeholder="🔍 Rechercher référence, titre, entreprise..."
            value={search}
      ...
      <div style={{ 
        ...
        <div>
          ...
          <p style={{ margin: 0, color: '#666' }}>Offres totales</p>
        </div>
        <div>
      ...
      <div style={{ background: 'white', borderRadius: '8px', overflow: 'hidden', boxShadow: '0 2px 8px rgba(0,0,0,0.1)' }}>
              ...
              <tr key={job.id} style={{ borderBottom: '1px solid #eee' }}>
                ...
                <td style={{ padding: '15px' }}>
                  <span style={{
                    ...
                    fontSize: '12px',
                    background: 
                      job.status === 'active' ? '#e6f7e6' :
                      job.status === 'published' ? '#e6f0ff' : '#fff4e6',
                    color: 
                      job.status === 'active' ? '#00a000' :
                      job.status === 'published' ? '#0066cc' : '#cc8400'
                  }}>
                    {job.status === 'active' ? 'ACTIVE' :
                     job.status === 'published' ? 'PUBLIÉE' : 'EN ATTENTE'}
                  </span>
                </td>
                <td style={{ padding: '15px' }}>
                  <div style={{ display: 'flex', gap: '10px' }}>
                    ...
                    <a 
                      href={`/checker?ref=${job.reference}`}
                      target="_blank"
                      style={{
                        padding: '5px 10px',
                        background: '#17a2b8',
        ...
        {filteredJobs.length === 0 && (
          <div style={{ padding: '40px', textAlign: 'center', color: '#666' }}>
            {jobs.length === 0 ? 'Aucune offre créée' : 'Aucun résultat pour cette recherche'}
          </div>
        )}
      </div>
    </div>
